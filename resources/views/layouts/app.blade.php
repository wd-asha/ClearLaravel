    ...
    <div class="footer">
        ...
        <div class="footer-item-form">
            <form action="{{ route('subscribe') }}" method="POST">
                @csrf
                <input type="text" name="user_email" class="footer-input" placeholder="Ваш e-mail">
                    @error('user_email')
                        <div style="color: white; font-size: .9rem; width: 100%; transform: translateY(.5rem);">
                            <p style="text-align: center; width: 100%; margin: 0; padding: 0">{{ $message }}</p>
                        </div>
                    @enderror
                <button type="submit" name="submit" class="footer-btn">Подписаться</button>
            </form>
        </div>
    </div>
</div>
        ...
        <div class="popup_content">
            ...
            <div class="popup_text">
                <form class="login-form" action="{{ route('login') }}" method="POST">
                    @csrf
                    <input class="login-input" name="email" type="email" placeholder="Email *" onfocus="this.value=''" autofocus>
                        @error('email')
